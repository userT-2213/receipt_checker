...
### 1. 支出の手動登録・編集確認機能
* 日付、店舗名、品名（複数行対応）、金額、カテゴリを管理・確認して登録できること。
* 日付入力はカレンダーUI（`showDatePicker`）による選択を強制し、表記のブレを防止。

### 2. Gemini APIを駆使したAIレシート画像解析補助機能
* **マルチモーダル解析（Gemini API）**: アップロードされたレシート画像から「取引日付」「店名」「購入した商品リスト」「合計金額（税込）」「適切な家計簿カテゴリ」をAIが統合的に判別。
...
| 3 | レシート登録 | 異常 | カメラ起動時の権限拒否 | 端末の設定でアプリのカメラ権限を「許可しない」にした状態で、「カメラで撮影」ボタンをタップする。 | エラーがキャッチされ、「カメラを起動できませんでした。権限の設定を確認してください。」という赤色のスナックバーが表示されること。 | o |
| 4 | 支出情報の入力 | 正常 | 日付選択ダイアログからの入力 | 日付入力欄をタップしてカレンダーダイアログを開き、任意の日付を選択して「OK」を押す。 | 選択した日付が「YYYY-MM-DD」形式で正確に日付入力欄に反映されること。 | o |
| 5 | 支出情報の入力 | 境界 | 日付選択の最小値（境界値） | 日付選択ダイアログでスクロールし、選択可能な最も古い日付である「2000-01-01」を選択する。 | 2000-01-01が入力欄に設定され、バリデーションエラーにならずに保持されること。 | o |
...
| # | 原因 | 対応 | 再テスト結果 |
|---|---|---|---|
| - | 画像アップロードの際に、Gemini API側のエラー（GenerativeAIException: Server Error [503] ... "status": "UNAVAILABLE"）が発生した。 | APIサーバー側の過負荷または一時的な利用制限（503 Unavailable）であったため、少し時間を置いてからリクエストを再試行するよう確認。 | 正常に疎通し、レシート解析が完了（合格） |
---
